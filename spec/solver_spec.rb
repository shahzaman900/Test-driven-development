...
describe Solver do
    ...
    context 'factorial' do
        ...
        it 'return factorial of 0 is 1'
           expect(method.factorial(0)).to eq(1)
        end
        it 'negative integer raise an exception.'
           expect(method.factorial(-3)).to raise(ArgumentError)
        end
    end

    context 'reverse' do
        it 'return reverse of a word'
           expect(method.reverseWord('hello')).to eq('olleh')
        end
    end
    ...
    context 'fizzbuzz' do
        ...
        it 'return buzz when number is divisible only by 5'
           expect(method.factorial(5)).to eq('buzz')
        end
        it 'return fizzbuzz when number is divisible  by 3 and 5'
